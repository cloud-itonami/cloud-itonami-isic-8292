(ns packagingops.operation
  "OperationActor -- one coordination request = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (PackagingOpsAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the ContractPackagingGovernor
  ...
  human packaging-operations coordinator/quality-assurance reviewer.
...
(defn build
  ...
  `packagingops.store/Store`).
  opts:
    :advisor      -- a `packagingops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; PackagingOpsAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; ContractPackagingGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
          ...
          (if (= :approved (:status approval))
            {:disposition :commit
             ...
             :audit [{:t :approval-granted :op (:op request)
                      :facility-id (:facility-id request)
                      :contract-id (:contract-id request) :by (:by approval)}]}
            {:disposition :hold
